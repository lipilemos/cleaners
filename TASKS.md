...
| T20b | Tela de criar conta em `client-app` (User) e `professional-portal` (Professional) — `RegisterFormComponent` compartilhado (`libs/shared/ui`), `SessionStore.register()`, endpoint por app via `REGISTER_PATH` (`register` / `professional-register`) — 🔗 **API** `POST /register`, `POST /professional-register` | ✅         | T18, T19   |
...
### Nota de ambiente: `nx serve` (Fase 6)

`nx serve client-app`/`professional-portal` (dev-server Vite-based do `@angular/build`) atualmente **crasha**: o otimizador de dependências do Vite tenta resolver `@cleaners/*` (libs symlinked em `node_modules/@cleaners/*` via npm workspaces) como pacotes npm reais e falha em imports relativos sem extensão (`./lib/foo`) — isso não acontece no `build` de produção nem nos testes (Vitest), que resolvem essas libs de outra forma. Não foi encontrada uma correção via config (`prebundle.exclude`, `preserveSymlinks`, tsconfig `include`) dentro do tempo investido; parece um limite real do Vite para pacotes de workspace que são TS puro (sem dist pré-compilado).
**Como rodar/testar mesmo assim**: `nx build <app>` (produção, funciona normalmente) + `nx run <app>:serve-static` — usado inclusive pelos specs e2e (`apps/*-e2e/playwright.config.mts`, configuração de build `e2e` com `environment.e2e.ts` para evitar CORS). Se `nx serve` for necessário no futuro (HMR), considerar pré-compilar as libs (`ng-packagr`/tsc build real com dist) em vez de consumi-las como TS fonte via symlink.
