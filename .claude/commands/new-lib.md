...
3. Aplique as tags corretas (`scope:shared,type:<tipo>`) no `project.json` gerado, e confirme que a regra de `@nx/enforce-module-boundaries` no `.eslintrc.json` da raiz já cobre essa combinação — se não cobrir, atualize a regra.
